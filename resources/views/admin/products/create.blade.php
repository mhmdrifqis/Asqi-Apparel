...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-black uppercase tracking-tight text-slate-900">Add Product</h1>
        <p class="text-slate-500 mt-1 text-sm">Create a new product with variants and images</p>
    </div>
...
<div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 sm:p-8" x-data="productForm()">
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" @submit="updateOrder">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div class="md:col-span-2">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            ...
            <div>
                ...
                <input type="number" name="base_price" value="{{ old('base_price', 0) }}" required min="0" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            ...
            <div>
                ...
                <input type="number" name="sale_price" value="{{ old('sale_price') }}" min="0" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            ...
            <div>
                ...
                <input type="number" name="weight_grams" value="{{ old('weight_grams', 500) }}" required min="0" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            ...
            <div>
                ...
                <input type="text" name="sport_type" value="{{ old('sport_type') }}" placeholder="e.g. Running, Training" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            </div>
            
            <div class="md:col-span-2">
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Short Description</label>
                <input type="text" name="short_description" value="{{ old('short_description') }}" maxlength="255" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">
            </div>

            <div class="md:col-span-2">
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Full Description</label>
                <textarea name="description" rows="4" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-3 text-sm focus:ring-accent focus:border-accent">{{ old('description') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Primary Image <span class="text-danger">*</span></label>
                <input type="file" name="primary_image" accept="image/jpeg,image/png,image/webp" required class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-2 text-sm focus:ring-accent focus:border-accent">
                @error('primary_image') <p class="text-danger text-xs mt-1 font-medium">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Gallery Images (Multiple)</label>
                <input type="file" name="gallery_images[]" multiple accept="image/jpeg,image/png,image/webp" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-2 text-sm focus:ring-accent focus:border-accent">
                @error('gallery_images.*') <p class="text-danger text-xs mt-1 font-medium">{{ $message }}</p> @enderror
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-2">Size Guide Image (Optional)</label>
                <input type="file" name="size_guide" accept="image/jpeg,image/png,image/webp" class="w-full bg-slate-50 border border-slate-200 text-slate-900 rounded-lg p-2 text-sm focus:ring-accent focus:border-accent">
                <p class="text-xs text-slate-500 mt-1">Upload a size chart specific to this product.</p>
        ...
        <div id="variants-list" class="space-y-4 mb-8">
            <template x-for="(variant, index) in variants" :key="variant.id">
                <div class="flex flex-wrap md:flex-nowrap gap-4 items-start bg-slate-50 p-4 border border-slate-200 rounded-lg relative overflow-hidden">
                    ...
                    <input type="hidden" :name="`variants[${index}][temp_id]`" :value="variant.id">
                    <input type="hidden" class="variant-row-id" :value="variant.id">
                    ...
                    <div class="w-full md:w-1/6">
                        <div class="flex justify-between items-center mb-1">
                            ...
                            <button type="button" @click="generateSku(index)" class="text-[9px] font-bold text-accent hover:underline flex items-center gap-0.5" title="Auto Generate SKU">
                        ...
                        <input type="text" :name="`variants[${index}][sku]`" x-model="variant.sku" required class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                    </div>
                    <div class="w-full md:w-1/6">
                        ...
                        <input type="text" :name="`variants[${index}][size]`" x-model="variant.size" placeholder="S, M, L, 40, 41" required class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                    </div>
                    <div class="w-full md:w-1/6">
                        ...
                        <input type="text" :name="`variants[${index}][color]`" x-model="variant.color" placeholder="Black" required class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                    </div>
                    <div class="w-full md:w-1/6">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Color Hex</label>
                        <div class="flex gap-2">
                            <input type="color" :name="`variants[${index}][color_hex]`" x-model="variant.color_hex" class="h-9 w-9 rounded cursor-pointer border-0 p-0">
                            <input type="text" x-model="variant.color_hex" class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                        </div>
                    </div>
                    <div class="w-full md:w-1/6">
                        ...
                        <input type="number" :name="`variants[${index}][stock]`" x-model="variant.stock" required min="0" class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                    </div>
                    <div class="w-full md:w-1/6">
                        <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">Price Adj (+/-)</label>
                        <input type="number" :name="`variants[${index}][price_adjustment]`" x-model="variant.price_adjustment" placeholder="0" class="w-full bg-white border border-slate-200 text-slate-900 rounded p-2 text-sm">
                    </div>
                    <div class="pt-5">
            ...
            <div x-show="variants.length === 0" class="text-center py-6 border-2 border-dashed border-slate-300 rounded-lg text-slate-500 text-sm">
                No variants added yet. Click "Add Variant" to add sizes and colors.
            </div>
        </div>
        ...
        <div class="flex items-center gap-8 p-4 bg-slate-50 rounded-lg border border-slate-200 mb-8">
            <div class="flex items-center">
                ...
                <input type="checkbox" name="is_active" id="is_active" value="1" checked class="w-5 h-5 text-accent border-slate-300 rounded focus:ring-accent">
                <label for="is_active" class="ml-2 block text-sm font-bold text-slate-700">Active (Visible in Store)</label>
            </div>
            <div class="flex items-center">
                <input type="hidden" name="is_featured" value="0">
                <input type="checkbox" name="is_featured" id="is_featured" value="1" class="w-5 h-5 text-accent border-slate-300 rounded focus:ring-accent">
...
<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('productForm', () => ({
            variants: [
                { id: Date.now(), sku: '', size: '', color: '', color_hex: '#000000', stock: 10, price_adjustment: 0 }
            ],
            ...
            updateOrder() {
                let order = [];
                document.querySelectorAll('.variant-row-id').forEach(el => {
                    order.push(el.value);
                });
